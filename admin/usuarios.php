...
<head>
    ...
    <title>FarmApp - Gestión de Usuarios</title>
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    if($usuarios->num_rows > 0):
                                        while($user = $usuarios->fetch_object()):
                                            // CORRECCIÓN: Verificar si las propiedades existen antes de usarlas
                                            $telefono = isset($user->telefono_usuario) ? $user->telefono_usuario : 'N/A';
                                            // CORREGIDO: Usar el nombre correcto de la columna de fecha
                                            $fecha_registro = isset($user->fecha_registro) ? $user->fecha_registro : (isset($user->fecha_creacion) ? $user->fecha_creacion : date('Y-m-d'));
                                            $nombre_rol = isset($user->nombre_rol) ? $user->nombre_rol : 'Usuario';
                                    ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge <?= $user->id_rol_usuario == 1 ? 'bg-primary' : 'bg-secondary' ?>">
                                                <?= htmlspecialchars($nombre_rol) ?>
                                            </span>
                                        </td>
                                        <td><?= date('d/m/Y', strtotime($fecha_registro)) ?></td>
                                        <td>
                                            <button class="btn btn-sm btn-outline-primary me-1" title="Editar">
                                                <i class="bi bi-pencil"></i>
                                            </button>
                                            <button class="btn btn-sm btn-outline-warning me-1" title="Cambiar Rol">
                    ...
                    <div class="col-md-3">
                        <div class="card bg-success text-white">
                            <div class="card-body text-center">
                                <h4>
                                    <?php
                                    $query = "SELECT COUNT(*) as total FROM usuarios WHERE id_rol_usuario = 1";
                                    $result = $db->query($query);
                                    echo $result->fetch_object()->total;
                                    ?>
                                </h4>
                    ...
                    <div class="col-md-3">
                        <div class="card bg-info text-white">
                            <div class="card-body text-center">
                                <h4>
                                    <?php
                                    $query = "SELECT COUNT(*) as total FROM usuarios WHERE id_rol_usuario = 2";
                                    $result = $db->query($query);
                                    echo $result->fetch_object()->total;
                                    ?>
                                </h4>
                    ...
                    <div class="col-md-3">
                        <div class="card bg-warning text-white">
                            <div class="card-body text-center">
                                <h4>
                                    <?php
                                    // CORRECCIÓN: Consulta simplificada sin fecha_registro
                                    $query = "SELECT COUNT(*) as total FROM usuarios";
                                    $result = $db->query($query);
                                    echo $result->fetch_object()->total;
                                    ?>
                                </h4>
                                <p class="mb-0">Todos los Usuarios</p>
                            </div>
                        </div>
                    </div>
                </div>
